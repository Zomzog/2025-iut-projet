...
Le prenom doit faire entre 2 et 20 caractères et ne peut pas être vide.

L'age doit être entre 18 et 119 ans.
...
Retourne la liste des familles enregistrées. Le paramètre `name` est optionnel et permet de filtrer les personnes par nom de famille si il est présent.
...
Si la personne a des réservations en cours, elle doivent etre supprimées avant de supprimer la personne.

== Reservation

Un reservation se présente sous la forme suivante :
...
L'ID est un UUID généré par le service lors de la création de la réservation, jamais pas l'utilisateur.
...
- `201` avec la réservation est créé.
...
==== `GET /api/v1/reservations?roomId=1&dayStart=2025-12-25&dayEnd=2025-12-26` : Permet de récupérer toutes les reservations

Les critères de filtrage `roomId`, `dayStart` et `dayEnd` sont tous optionnels.

Il faut respecter la règle si elles sont fourni: `dayStart <= dayEnd`.
...
Permet de mettre à jour un réservation.
...
Permet de supprimer un reservation.
