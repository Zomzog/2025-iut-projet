...
Une collection avec des appels de base link:https://www.usebruno.com/[bruno] est fournie pour tester les services.
...
Ici, il fait l'agrégation des données des deux autres services.
...
Son ID est un Long qui sera fourni par la BDD lors de la création de la personne, jamais par l'utilisateur.
...
Retourne la liste des personnes enregistrées. Le paramètre `name` est optionnel et permet de filtrer les personnes par nom de famille s'il est présent.
...
- `400` si le format de l'id est inv
alide.
...
* Ce service doit fournir une implémentation de la "base de données" en h2. (/!\ end et day sont des mots-clés réservés en h2, il faut utiliser un autre nom de colonne).
...
==== `POST /api/v1/reservations` : Permet de créer une réservation
...
Dans le cadre de ce projet, seuls sont requis un endpoint de création de user (au sens spring-security) et deux endpoints d'agrégation.
...
* Le BFF n'a pas de cache, il appelle toujours les autres services.
